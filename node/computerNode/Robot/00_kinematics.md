...
先思考旋转姿态关系，只需要思考如何旋转使得两坐标系的XZ平面共面。  
如果只是要XY共面，是不是就只需要两次旋转？ 好了，就这么XY一共面，双方姿态就完全相同了！          
你说，你放屁，Y轴方向可能指向XZ平面分割空间的上面也可能指向XZ平面分割空间的令一面！凭什么对齐XZ平面就对齐了两坐标系的姿态？    

**对齐XZ平面就对齐了两坐标系的姿态的原因在于各个关节坐标系的Y轴的方向都依据X轴，Y轴右手定则所描述，因此Y轴的方向是统一而绝对确定的！**     
**这才是DH表示法能用四个数表征空间坐标系对齐关系的灵魂所在**。       
好了，两次旋转，姿态对齐了对吧？ 欻欻两个位移 空间位置也给对齐了，两坐标系完全重合。  
