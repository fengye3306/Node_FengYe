...
执行串行轮询操作是指通过串行通信接口（如RS-232、USB等）向特定设备发送请求，以获取设备的状态或数据。
在这个过程中，主控设备（如计算机或微控制器）按照一定的顺序依次查询一个或多个从设备的状态，获取它们的响应信息。
这个过程通常涉及以下几个步骤：
建立通信连接：确保主控设备和从设备之间的串行连接已正确建立。

* 发送请求：主控设备向目标设备发送请求命令，要求其返回状态信息。
* 接收响应：目标设备在接收到请求后，处理该请求并返回相应的数据或状态信息。
* 处理数据：主控设备接收到响应后，解析和处理返回的数据，以获得所需的信息。
* 循环操作：如果需要监测多个设备，主控设备会依次对每个设备进行相同的操作，以实现轮询。

串行轮询操作的优点是可以同时监控多个设备的状态，便于进行集中管理和控制。不过，它也可能存在延迟问题，
尤其是在设备数量较多时，每个设备的响应时间会影响整体的轮询效率。
## 全局状态字
...
// 打印异常关键字  
{
    int int_errorCode = ThreadIberr();
    switch (int_errorCode) {
    case 0:  // EDVR
        qDebug() << "Error Code: 0 (EDVR) - 系统错误";
        break;
    case 1:  // ECIC
        qDebug() << "Error Code: 1 (ECIC) - 需要 GPIB 接口作为 CIC,但当前不是";
        break;
    case 2:  // ENOL
        qDebug() << "Error Code: 2 (ENOL) - GPIB 总线上没有监听者";
        break;
    case 3:  // EADR
        qDebug() << "Error Code: 3 (EADR) - GPIB 接口未正确寻址";
        break;
    case 4:  // EARG
        qDebug() << "Error Code: 4 (EARG) - 无效的函数参数";
        break;
    case 5:  // ESAC
        qDebug() << "Error Code: 5 (ESAC) - GPIB 接口未作为系统控制器";
        break;
    case 6:  // EABO
        qDebug() << "Error Code: 6 (EABO) - I/O 操作超时中止";
        break;
    case 7:  // ENEB
        qDebug() << "Error Code: 7 (ENEB) - 不存在的 GPIB 接口";
        break;
    case 8:  // EDMA
        qDebug() << "Error Code: 8 (EDMA) - DMA 错误";
        break;
    case 10:  // EOIP
        qDebug() << "Error Code: 10 (EOIP) - 异步 I/O 正在进行中";
        break;
    case 11:  // ECAP
        qDebug() << "Error Code: 11 (ECAP) - 设备不具备执行该操作的能力";
        break;
    case 12:  // EFSO
        qDebug() << "Error Code: 12 (EFSO) - 文件系统错误";
        break;
    case 14:  // EBUS
        qDebug() << "Error Code: 14 (EBUS) - GPIB 总线错误";
        break;
    case 16:  // ESRQ
        qDebug() << "Error Code: 16 (ESRQ) - SRQ 信号卡在 ON 状态";
        break;
    case 20:  // ETAB
        qDebug() << "Error Code: 20 (ETAB) - 表问题";
        break;
    case 21:  // ELCK
        qDebug() << "Error Code: 21 (ELCK) - 接口被锁定";
        break;
    case 22:  // EARM
        qDebug() << "Error Code: 22 (EARM) - ibnotify 回调未能重新触发";
        break;
    case 23:  // EHDL
        qDebug() << "Error Code: 23 (EHDL) - 输入句柄无效";
        break;
    case 26:  // EWIP
        qDebug() << "Error Code: 26 (EWIP) - 等待指定的输入句柄操作";
        break;
    case 27:  // ERST
        qDebug() << "Error Code: 27 (ERST) - 事件通知由于接口重置被取消";
        break;
    case 28:  // EPWR
        qDebug() << "Error Code: 28 (EPWR) - 接口丢失电源";
        break;
    default:
        qDebug() << "Error Code: " << int_errorCode << " 未知错误";
        break;
    }
}
```
...
```cpp
/* nullptr指针置空却引发全局异常状态字置为 - (EARG 无效的函数参数)
    直接将空指针传递给 ibask 会导致函数无法写入正确的值。
*/
