...
它的平滑效果通过对图像进行加权平均实现，权重由高斯函数（或称为正态分布）决定。高斯滤波器以其平滑效果和**较好的边缘保留能力**而闻名，广泛应用于图像处理中的噪声去除和预处理。  
...
**双边滤波器（Bilateral Filter）是一种非线性滤波器，** 用于在保留图像边缘的同时进行去噪和平滑。  
它在进行空间加权的同时，结合像素值之间的相似性，使得在去除噪声的同时能够保留更多的图像细节。双边滤波器在图像处理、计算机视觉中有广泛的应用，如HDR图像处理和图像细节增强。
